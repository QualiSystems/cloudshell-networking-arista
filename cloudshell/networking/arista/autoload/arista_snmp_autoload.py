...
from cloudshell.devices.standards.networking.autoload_structure import GenericResource, GenericChassis, \
    GenericPowerPort, GenericModule, GenericSubModule, GenericPort, GenericPortChannel
from cloudshell.shell.core.driver_context import AutoLoadDetails
from cloudshell.snmp.quali_snmp import QualiSnmp

from cloudshell.networking.arista.autoload.snmp_entity_table import SNMPEntityTable
from cloudshell.networking.arista.autoload.snmp_if_table import SnmpIfTable


class AristaSNMPAutoload(object):
    IF_ENTITY = "ifDescr"
    ...
    PRODUCT_MIBS = ['ARISTA-PRODUCTS-MIB']
    VENDOR = 'Arista'
    ...
    def _load_snmp_tables(self):
        ...
        self.if_table = SnmpIfTable(self.snmp_service, self.logger)
        self.logger.info('{0} table loaded'.format(self.IF_ENTITY))
        self.arista_entity = SNMPEntityTable(self.snmp_service, self.logger, self.if_table)
        self.entity_table = self.arista_entity.get_entity_table()
    ...
    def _get_chassis_attributes(self, chassis_list):
        ...
        for chassis in chassis_list:
            chassis_id = self.arista_entity.relative_address[chassis]
            ...
            chassis_object.model = self.snmp_service.get_property("ENTITY-MIB", "entPhysicalModelName", chassis) or \
                                   self.entity_table[chassis]["entPhysicalDescr"]
            chassis_object.serial_number = self.snmp_service.get_property("ENTITY-MIB", "entPhysicalSerialNum", chassis)
    ...
    def _get_module_attributes(self, module_list):
        ...
        for module in module_list:
            module_id = self.arista_entity.relative_address.get(module)
            if not module_id:
                continue
            ...
            module_object.version = self.snmp_service.get_property("ENTITY-MIB", "entPhysicalSoftwareRev", module)
            module_object.serial_number = self.snmp_service.get_property("ENTITY-MIB", "entPhysicalSerialNum", module)
    ...
    def _get_power_ports(self, power_supply_list):
        """Get attributes for power ports provided in power_supply_list
        ...
        for port in power_supply_list:
            ...
            chassis_id = self.arista_entity.get_relative_address(parent_index)
            ...
            power_port.model = self.snmp_service.get_property("ENTITY-MIB", "entPhysicalModelName", port)
            power_port.port_description = self.snmp_service.get_property("ENTITY-MIB", "entPhysicalDescr", port)
            power_port.version = self.snmp_service.get_property("ENTITY-MIB", "entPhysicalHardwareRev", port)
            power_port.serial_number = self.snmp_service.get_property("ENTITY-MIB", "entPhysicalSerialNum", port)
    ...
    def _get_ports_attributes(self, port_list):
        """Get resource details and attributes for every port in port_list"""

        self.logger.info("Load Ports:")
        for port in port_list:
            port_if_entity = self.arista_entity.port_mapping.get(port)
            ...
            self._add_element(relative_path=self.arista_entity.relative_address[port], resource=port_object)
    ...
    def _get_device_os_version(self):
        """Get device OS Version from SNMPv2 mib
    ...
    def _get_device_model(self):
        ...
        match_name = re.search(r'::(?P<model>\S+$)', self.snmp_service.get_property('SNMPv2-MIB', 'sysObjectID', '0'))
        return match_name.groupdict().get('model', '').capitalize()
