...
class AristaEnableDisableSNMPFlow(EnableDisableSnmpFlowInterface):
    def __init__(self, cli_configurator: AristaCLIConfigurator, logger: Logger, vrf_name=None):
        self._cli_configurator = cli_configurator
        self._logger = logger
        self.vrf_name = vrf_name

    def enable_snmp(self, snmp_parameters: Union[SNMPReadParameters, SNMPWriteParameters, SNMPV3Parameters]):
        if (
        ...
        with self._cli_configurator.config_mode_service() as cli_service:
            ...
            # if self._vrf_name and not snmp_actions.is_configured_vrf(
            #         self._vrf_name
            # ):
    ...
    def disable_snmp(self, snmp_parameters: Union[SNMPReadParameters, SNMPWriteParameters, SNMPV3Parameters]):
        if (
            hasattr(snmp_parameters, "snmp_community")
            and not snmp_parameters.snmp_community
        ):
